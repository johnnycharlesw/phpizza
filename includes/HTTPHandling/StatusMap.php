<?php
namespace PHPizza\HTTPHandling;

/**
 * StatusMap — small helper to map HTTP status codes to reason phrases
 * and provide small helpers.
 */
class StatusMap
{
    protected array $map = [
        100 => 'Continue',
        101 => 'Switching Protocols',
        200 => 'OK',
        201 => 'Created',
        202 => 'Accepted',
        203 => 'Non-Authoritative Information',
        204 => 'No Content',
        205 => 'Reset Content',
        206 => 'Partial Content',
        300 => 'Multiple Choices',
        301 => 'Moved Permanently',
        302 => 'Found',
        303 => 'See Other',
        304 => 'Not Modified',
        307 => 'Temporary Redirect',
        308 => 'Permanent Redirect',
        400 => 'Bad Request',
        401 => 'Unauthorized',
        402 => 'Payment Required',
        403 => 'Forbidden',
        404 => 'Not Found',
        405 => 'Method Not Allowed',
        406 => 'Not Acceptable',
        407 => 'Proxy Authentication Required',
        408 => 'Request Timeout',
        409 => 'Conflict',
        410 => 'Gone',
        411 => 'Length Required',
        412 => 'Precondition Failed',
        413 => 'Payload Too Large',
        414 => 'URI Too Long',
        415 => 'Unsupported Media Type',
        416 => 'Range Not Satisfiable',
        417 => 'Expectation Failed',
        418 => "I'm A Teapot",
        422 => 'Unprocessable Entity',
        425 => 'Too Early',
        426 => 'Upgrade Required',
        428 => 'Precondition Required',
        429 => 'Too Many Requests',
        431 => 'Request Header Fields Too Large',
        451 => 'Unavailable For Legal Reasons',
        500 => 'Internal Server Error',
        501 => 'Not Implemented',
        502 => 'Bad Gateway',
        503 => 'Service Unavailable',
        504 => 'Gateway Timeout',
        505 => 'HTTP Version Not Supported',
    ];

    public static function getReason(int $code): string
    {
        return self::$map[$code] ?? 'Unknown Status';
    }

    public function isClientError(int $code): bool
    {
        return $code >= 400 && $code < 500;
    }

    public function isServerError(int $code): bool
    {
        return $code >= 500 && $code < 600;
    }

    public function isSuccessful(int $code): bool
    {
        return $code >= 200 && $code < 300;
    }

    public function isRedirect(int $code) : bool
    {
        return $code >= 300 && $code < 400;
    }

    public function isStandardCode(int $code): bool
    {
        $check1 = $code >= 100 && $code < 600;
        $check2 = array_has(array_keys($this->map),code);
        $final = $check1 && $check2;
        return $final;
    }

    /**
     * Send the status header for the given code and optional body.
     * Note: this function will echo the body if provided.
     */
    public function send(int $code, string $body = null): void
    {
        $reason = $this->getReason($code);
        if (!headers_sent()) {
            header(sprintf('HTTP/1.1 %d %s', $code, $reason), true, $code);
        }
        if ($body !== null) {
            echo $body;
        }
    }
}
