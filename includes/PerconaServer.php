...
class PerconaServer implements SQLDatabaseManagementSystemDriver {
    ...
    public function __construct($dbServer, $dbUser, $dbPassword, $dbName){
        $this->__construct($dbServer, $dbUser, $dbPassword, $dbName);
    }

    public function ___construct($dbServer, $dbUser, $dbPassword, $dbName) {
    ...
    public function getLastInsertId() {
        global $percona_server__connection;
        return $percona_server__connection->insert_id;
    }
...
HTML;
    ...
    if (!$percona_server__connection->set_charset("utf8mb4")) {
        error_log("Error loading character set utf8mb4: " . $percona_server__connection->error);
        die("Database configuration error. Please contact the administrator.");
    }
}
function run_query($query, $params = [], $types = ''){
    global $percona_server__connection;
    $stmt = $percona_server__connection->prepare($query);
    if ($stmt === false) {
        error_log("Prepare failed: " . $percona_server__connection->error . " -- Query: " . $query);
...
function close_database(){
    global $percona_server__connection;
    if ($percona_server__connection) {
        $percona_server__connection->close();
    }
}
